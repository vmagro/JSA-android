 ...
 * {@link ListView}s. <br />
 * <b>NOTE: The adapter assumes that the data source of the decorated list
 * adapter is sorted.</b>
 * 
...
public class SimpleSectionAdapter<T> extends BaseAdapter {
	...
	private LinkedHashMap<String, Integer> mTimes;
	...
	public SimpleSectionAdapter(Context context, BaseAdapter listAdapter,
		...
		this.mTimes = new LinkedHashMap<String, Integer>();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		case VIEW_TYPE_SECTION_HEADER:
			if (view == null) {
				...
				sectionHolder.timeTextView = (TextView) view
		...
		if (sectionHolder != null) {
			...
			sectionHolder.timeTextView
					.setText(sectionTimeForPosition(position));
			Log.i("topic", sectionName + "," + sectionTimeForPosition(position));
		}
	...
	private void findSections() {
		...
		for (int i = 0; i < n; i++) {
			...
			@SuppressWarnings("unchecked")
			String time = mSectionizer.getTime((T) mListAdapter.getItem(i));

			if (!mSections.containsKey(sectionName)){
				mSections.put(sectionName, i + nSections);
				mTimes.put(time, i + nSections);
	...
	private String sectionTimeForPosition(int position) {
		String time = "";

		Set<Entry<String, Integer>> entrySet = mTimes.entrySet();
		for (Entry<String, Integer> entry : entrySet) {
			if (entry.getValue() == position) {
				time = entry.getKey();
				break;
